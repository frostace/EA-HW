...
void hill_climber()
{
    ...
    // #pragma omp parallel
    {
        ...
        int patrolNums = 4; // number of parallel climbing patrols
        ...
        // #pragma omp for
        for (int i = 0; i < 100000 / patrolNums; i++)
        {
            // id = omp_get_thread_num();
            for (int id = 0; id < patrolNums; id++)
            {
                ...
                if (patrols[id].scaledCentroidX > bestDistance)
                {
                    ...
                    bestRobotTypes = patrols[id].saveSkeleton();
                }

                // if stuck times > threshold, refresh patrol
