...
public:
    ...
    void iterate()
    {
        iterateHelper(true, iterNums);
        iterNums += 1;
    }
    void iterate(bool breathing)
    {
        iterateHelper(breathing, iterNums);
        iterNums += 1;
    }

    void iterateHelper(bool breathing, int iterNums)
    {
        ...
        for (auto &cube : cubes)
        {
            cube.iterate(breathing, iterNums);
        }
        for (auto &cube : cubes)
        {
            cube.integrate();
    ...
    void draw(VertexArray &va, VertexBuffer &vb_pos, VertexBuffer &vb_norm, Shader &shader)
    {
        for (auto &cube : cubes)
        {
            if (cube.currType == 4)
                continue;
            shader.SetUniform3fv("u_Color", cube.color);

            vb_pos.Update(cube.positions, sizeof(cube.positions));

            va.SpecifyBufferLayout(0, vb_pos);
            va.SpecifyBufferLayout(2, vb_norm);
