...
public:
    ...
    void computeFitness(Population &opponents)
    {
        ...
        for (int i = 0; i < popSize; i++)
        {
            while (idx_j < popSize && opponents.population[idx_j].scaledCentroidX * opponents.population[idx_j].scaledCentroidZ >= population[i].scaledCentroidZ * population[i].scaledCentroidX)
            {
    ...
    float distance(Creature &robot1, Creature &robot2)
    {
        ...
        for (int i = 0, len = robot1.cubes.size(); i < len; i++)
        {
            ...
            // TODO: modified on 12-02
            // v1.push_back(robot1.cubes[i].currType);
            // v2.push_back(robot2.cubes[i].currType);
            v1.push_back(robot1.cubes[i].springs[0]->k);
            v2.push_back(robot2.cubes[i].springs[0]->k);
        }
